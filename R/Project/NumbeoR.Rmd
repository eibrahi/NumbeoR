...
* Analysis of perceived crime
    * global analysis of Numbeo data
    * hypothesis analysis with focus of Germany
...
## Data Sources
Numbeo
Statista for real crime rates and number of applications for asylum in Germany
...
```{r}
drops <- c("Year","Country", "Continent")
df_master_numeric <- df_analysis_clean[ , !(names( df_analysis_clean) %in% drops)]
#df_master_numeric <- na.omit(df_master_numeric)
```
...
```{r}
corN <- rcorr(as.matrix(select(df_analysis_numeric,`Crime Index`,`Cost of Living Index`)))
corN
```
...
```{r echo=FALSE}
corN <- rcorr(as.matrix(select(df_analysis_numeric,`Crime Index`,`Local Purchasing Power Index`)))
corN
```
...
```{r echo=FALSE}
corN <- rcorr(as.matrix(select(df_analysis_numeric,`Crime Index`, `CO2 Emission Index`)))
corN
```
...
corN <- rcorr(as.matrix(select(df_corCrimeAsylumGer, `Crime Index`, `AsylumSeekerGermanyCum`)))
corN
```
...
# Correlation analysis
corN <- rcorr(as.matrix(select(df_corCrimePerRealGer, `Crime Index`, `number_crimes`)))
corN
```
...
crimeTrafficPollution_plot_2019 <- ggplot(df_analysis_clean_2019) +
    geom_point(
        mapping = aes(x = `Traffic Index`, y = `Pollution Index`, size=`Crime Index`,color= Continent, points=Country)
        )
...
fit_ets2 <- tsibble_Germany %>%
  model(ETS(`Crime Index` ~ error("A") + trend("A") + season("N"), opt_crit = "mse"))
fc_ets2 <- fit_ets2 %>%
  forecast(h = 3)
