...
```{r}
glimpse(df_master_continent[1:2,1:10])
```
...
```{r}
glimpse(df_master_continent[1:3,])
```
...
Selection of some variables which are to be used in some of the analyses (e.g correlation analysis), storage in a analysis dataframe.
...
```{r}
glimpse(df_master_continent[1:3,1:5])
```
